...
# FIX 1: nmap wrapped in try/except so module-level crash is avoided
try:
    import nmap
    NMAP_AVAILABLE = True
except Exception:
...
logging.info(f"nmap    : {'available' if NMAP_AVAILABLE else 'NOT available — scanner disabled'}")
logging.info(f"scapy   : {'available' if SCAPY_AVAILABLE else 'NOT available — sniffer disabled'}")


# ===================== GLOBAL LOCKS =======================

# FIX 3: Use a single RLock (reentrant) for the database so that code paths
# which already hold engine.lock can safely call db methods without deadlock.
db_lock    = threading.RLock()   # was threading.Lock() — caused potential deadlock
...
class DatabaseManager:
    """Handles all SQLite operations. Thread-safe via RLock."""
...
class ThreatEngine:
    """
    Central SOC brain.
    - Maintains per-IP risk score
    - Tracks last seen activity
    - Correlates attack patterns
    - Escalates severity automatically
    - Feeds alerts to database

    Thread-safety: all public methods acquire self.lock.
    db_lock is RLock so it's safe to call db methods while holding self.lock.
    """
    ...
    def _escalate(self, ip, reason):
        # Called INSIDE self.lock — db_lock is RLock so this is safe
    ...
    def add_event(self, ip, weight, event_type):
        with self.lock:
            ...
            # Persist raw event (db_lock is RLock — safe to acquire here)
    ...
    def get_score(self, ip):
        """FIX 2/7/8: Thread-safe score read — used by monitors instead of direct dict access."""
        with self.lock:
            return self.risk_scores[ip]
        ...
        with self.lock:
            for ip in list(self.last_seen.keys()):
                if now - self.last_seen[ip] > ttl:
                    ...
                    # FIX 5: use pop with default — key may not exist in defaultdict
                    # if the deque was never populated for this IP
...
class AttackSimulator:
    """Generates controlled fake attacks for demo/testing."""
    ...
    def simulate_port_scan(self, ip):
        logging.warning(f"[SIM] Port scan started from {ip}")
        for _ in range(5):
            self.engine.add_event(ip, PORT_SCAN_WEIGHT, "PORT_SCAN")
            # FIX 2: use thread-safe get_score() instead of direct dict access
            score = self.engine.get_score(ip)
            msg   = f"[SIMULATED] Port scan from {ip}"
            db.save_alert("SIM_PORTSCAN", score, msg, ip)
            time.sleep(0.4)

    def simulate_ssh_bruteforce(self, ip):
        ...
        for _ in range(6):
            ...
            db.save_alert("SIM_SSH", score, msg, ip)
            time.sleep(0.6)

    def run_demo(self):
        # FIX 6: pick ONE ip per demo run so port scan + SSH scores accumulate on the same IP
        ip = random.choice(self.fake_ips)
        self.simulate_port_scan(ip)
        self.simulate_ssh_bruteforce(ip)
...
class SSHLogMonitor:
    ...
    def monitor(self):
        if not os.path.exists(self.log_path):
            logging.warning(f"SSH log not found at {self.log_path} — SSH monitoring disabled")
            return
        ...
        with open(self.log_path, "r", errors="ignore") as f:
            ...
            while True:
                ...
                if "Failed password" in line:
                    ...
                    # FIX 7: use thread-safe get_score() instead of direct dict access
                    score = self.engine.get_score(ip)
                    if score >= ALERT_THRESHOLD * SSH_FAIL_WEIGHT:
                        msg = f"SSH Bruteforce detected from {ip} ({geoip_lookup(ip)})"
                        db.save_alert("SSH_BRUTEFORCE", score, msg, ip)
                        logging.warning(msg)
...
class PacketAnalyzer:
    """Captures TCP packets and detects suspicious port access."""
    ...
    def handle_packet(self, pkt):
        ...
        if dst_port in MONITORED_PORTS:
            ...
            if self.port_hits[src_ip] == ALERT_THRESHOLD:
                # FIX 8: use thread-safe get_score() instead of direct dict access
                score = self.engine.get_score(src_ip)
                msg   = f"Port scan detected from {src_ip} ({geoip_lookup(src_ip)})"
                db.save_alert("PORT_SCAN", score, msg, src_ip)
                logging.warning(msg)

    def start(self):
        # FIX 9: guard is already present — but also log clearly when unavailable
        if not SCAPY_AVAILABLE:
            logging.warning("Scapy not available — packet sniffing disabled. "
                            "Install with: pip install scapy")
            return

        logging.info("Packet sniffer started (requires root/admin privileges)")
        try:
            sniff(filter="tcp", prn=self.handle_packet, store=False)
        except PermissionError:
            logging.error("PermissionError: run SOC as root to enable packet sniffing")
        except Exception as e:
...
class NetworkScanner:
    ...
    def __init__(self, network_range):
        ...
        # FIX 1: instantiate PortScanner only if nmap is available and installed on the system
        try:
            self.nm = nmap.PortScanner() if NMAP_AVAILABLE else None
        except Exception:
            logging.warning("nmap binary not found in system path. Network scanning is disabled.")
            self.nm = None

    def scan(self):
        if not NMAP_AVAILABLE or self.nm is None:
            logging.warning("nmap not available — network scanning disabled. "
                            "Install with: pip install python-nmap  (and nmap binary)")
            return
...
class MetricsCollector:
    """Collects system + SOC health metrics every METRIC_INTERVAL seconds."""
...
# FIX 4: Moved ABOVE Flask app so /report route can reference it safely

class SOCReportGenerator:
    """Generates SOC reports in TXT and JSON formats."""
    ...
    def generate_text_report(self, filename="soc_report.txt"):
        ...
        with open(filepath, "w") as f:
            ...
            if metrics:
                ...
                f.write("No metrics collected yet\n")
        ...
        logging.info(f"Text report: {filepath}")
        return filepath
    ...
    def generate_json_report(self, filename="soc_report.json"):
        ...
        logging.info(f"JSON report: {filepath}")
        return filepath
...
<head>
    ...
    <style>
        body { background:#020617; color:#e5e7eb; font-family:monospace; margin:0; padding:0; }
        .header { padding:20px; background:#020617; border-bottom:3px solid #22c55e; text-align:center; }
        .header h1 { color:#22c55e; margin:0; }
        .header h3 { color:#94a3b8; margin:5px 0; }
        .container { padding:20px; }
        .stats { display:flex; gap:20px; margin-bottom:20px; }
        .box { flex:1; background:#020617; border:1px solid #334155; padding:15px; text-align:center; }
...
<div class="container">
<div class="stats">
    <div class="box"><h3>Total Packets</h3><p>{{ packets }}</p></div>
    <div class="box"><h3>Events Processed</h3><p>{{ events }}</p></div>
    <div class="box"><h3>Active Threat IPs</h3><p>{{ threats }}</p></div>
</div>
<div class="stats">
...
<h2>Recent Alerts</h2>
<table>
<tr><th>Time</th><th>Category</th><th>Severity</th><th>Message</th><th>Source IP</th></tr>
{% for a in alerts %}
<tr>
    <td>{{ a[0] }}</td>
    <td>{{ a[1] }}</td>
...
def chrome_demo_attack():
    threading.Thread(target=attack_simulator.run_demo, daemon=True).start()
    return "<h3>Demo Attack Started</h3>"

@app.route("/report")
def chrome_generate_report():
    # FIX 4: report_generator now defined before this route is ever called
...
def start_web_dashboard():
    logging.info(f"Web dashboard on http://{WEB_HOST}:{WEB_PORT}")
...
def start_all_threads():
    threads = [
        threading.Thread(target=packet_analyzer.start,    daemon=True, name="PacketSniffer"),
        threading.Thread(target=metrics_collector.loop,   daemon=True, name="MetricsCollector"),
        threading.Thread(target=_network_scan_loop,       daemon=True, name="NetworkScanner"),
        threading.Thread(target=ssh_monitor.monitor,      daemon=True, name="SSHMonitor"),
        threading.Thread(target=demo_mode_loop,           daemon=True, name="DemoMode"),
        threading.Thread(target=start_web_dashboard,      daemon=True, name="WebDashboard"),
    ]
    for t in threads:
        t.start()
...
def print_banner():
 ...
 --- MINI SECURITY OPERATIONS CENTER ---
        Defensive Security | Blue Team
        Version: 1.1-FIXED
        (ASCII banner fallback due to encoding support)
    """)
...
def main():
    ...
    try:
        ...
        print("\n[!] CTRL+C — shutting down SOC")
        logging.warning("SOC shutdown initiated by user")

        # FIX 10: generate reports in main thread so they complete before exit
        try:
            logging.info("Generating final reports...")
            report_generator.generate_text_report()
            report_generator.generate_json_report()
            logging.info("Reports saved to: " + REPORT_DIR)
        except Exception as e:
